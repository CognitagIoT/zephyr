...
/* Different operations should poll the WIP bit at different rates */
enum wait_ready_op_type {
	WAIT_READY_REGISTER     = 0,
	WAIT_READY_WRITE        = 1,
	WAIT_READY_ERASE        = 2,
};
 ...
 * @param op_type Operation that we are waiting on
 * @return 0 on success, negative errno code otherwise
 */
static int spi_nor_wait_until_ready(const struct device *dev, enum wait_ready_op_type op_type)
{
	static const k_timeout_t op_delay[] = {
		/* Register writes should be ready extremely quickly */
		[WAIT_READY_REGISTER] = K_NO_WAIT,
		/* Page writes range from sub-ms to 10ms */
		[WAIT_READY_WRITE] = K_TICKS(1),
		/* Erases can range from 45ms to 240sec */
		[WAIT_READY_ERASE] = K_MSEC(50),
	};
	int ret;
	uint8_t reg;
