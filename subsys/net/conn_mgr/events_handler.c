					  ...
					  struct net_if *iface)
{
	...
	NET_DBG("%s event %x received on iface %d (%p)", "Iface", mgmt_event,
					 ...
					 struct net_if *iface)
{
	...
	NET_DBG("%s event %x received on iface %d (%p)", "IPv6", mgmt_event,
		net_if_get_by_iface(iface), iface);
					 ...
					 struct net_if *iface)
{
	...
	NET_DBG("%s event %x received on iface %d (%p)", "IPv4", mgmt_event,
		net_if_get_by_iface(iface), iface);
